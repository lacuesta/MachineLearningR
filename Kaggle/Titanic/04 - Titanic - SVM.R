...
# Same as 02 - Titanic - Try to use SVM
# Load datasets
...
response <- c("Survived")
library(caret)
titanic_svm_train <- train(Survived ~ ., data = titanic_train[c(predictors,response)], verbose = TRUE, "svmRadial")

titanic_svm_pred <- predict(titanic_svm_train, titanic_test)
titanic_test$Survived <- titanic_svm_pred
# Convert to 0's and 1's
titanic_test$Survived <- as.numeric(titanic_test$Survived) - 1
write.csv(titanic_test[c("PassengerId","Survived")], "test_predictions_svm.csv", row.names = FALSE)
